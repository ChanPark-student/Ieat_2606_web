...
def get_kc_summary(
    ...
    # query로 samples 정렬: 입력 제품과 연관도 높은 사례를 먼저 표시.
    # 정렬 우선순위 (모두 내림차순):
    #   1) 품목 키워드 그룹(인형/자동차/색연필 등) 매칭 — 정밀도 확보, 정규식 없이도
    #      "곰인형" 입력이 "테디베어" 샘플과 연결
    #   2) 임베딩 의미 유사도 — 키워드 그룹이 없는 품목/동점 사례를 의미로 정렬
    #      (모델명이 짧고 암호같아 임베딩 단독 변별력은 약하므로 보조 신호로만 사용)
    #   3) 원문 단어 겹침 — 최종 tie-break
    # 임베딩 미가용 시 2)는 0으로 폴백 → 기존 키워드 정렬과 동일하게 동작.
    ...
    if query_text and valid_samples:
        try:
            from app.search.embedding_search import similarity_scores
            passages = [
                f"{s.get('modelName') or ''} {s.get('productName') or ''}".strip()
                for s in valid_samples
            ]
            sims = similarity_scores(
                app_data, f"kc:{matched_kc_key}", query_text, passages
            )
            if sims:
                emb_by_id = {id(s): sims[i] for i, s in enumerate(valid_samples)}
        except Exception as e:
            logger.warning("Phase 6 임베딩 정렬 실패 (키워드 폴백): %s", e)

    if query_words or matched_groups or emb_by_id:
        samples_sorted = sorted(
            valid_samples,
            key=lambda s: (
                _group_relevance(s, matched_groups),
                emb_by_id.get(id(s), 0.0),
                _sample_relevance(s, query_words),
            ),
            reverse=True,
        )
    else:
        samples_sorted = valid_samples

...
def _build_checklist_signals(input_summary: dict, legal_name: str) -> dict:
    """입력 제품 특성(소재·전원·품목명)에서 체크리스트 필터·정렬용 신호를 추출.

    _filter_conditional_items / _score_checklist_item에서 공용으로 사용.
    """
    ...
    if show_heat_items and has_heat_signal:
        priority_kw += _HEAT_SIGNAL_TOKENS
    ...
    return {
        ...
        "show_heat_items": show_heat_items,
        "has_strap_signal": has_strap_signal,
    }
...
def _filter_conditional_items(items: List[str], signals: dict) -> List[str]:
    """온열/끈-코드류 항목을 트리거 신호 미충족 시 제외.

    폼알데하이드·아릴아민/아조염료 등 섬유 유해물질 항목은 절대 숨기지 않음(요구사항).
    전부 제거되어 빈 리스트가 되면 안전장치로 원본을 유지한다.
    """
    filtered: List[str] = []
    for item in items:
        item_l = item.lower()
        if _is_heat_item(item_l) and not signals["show_heat_items"]:
            continue
        if _is_strap_item(item_l) and not signals["has_strap_signal"]:
            continue
        filtered.append(item)
    return filtered if filtered else items


def _score_checklist_item(item: str, signals: dict) -> int:
    item_l = item.lower()
    p_score = sum(10 for kw in signals["priority_kw"] if kw in item_l)
    if p_score > 0:
        return p_score  # priority 신호 있으면 depriority 패널티 무시
    return sum(-5 for kw in signals["depriority_kw"] if kw in item_l)


def _build_unified_checklist(
    checklist: List[str],
    prevention_points: List[str],
    input_summary: dict,
    legal_name: str,
) -> List[Tuple[str, bool]]:
    """§7 출시 전 확인 체크리스트를 리콜 유래 항목 + 일반 항목으로 통합.

    prevention_points(실제 리콜에서 반복된 사유)는 경험적으로 증명된 위험이므로
    조건부 하드 필터(온열/끈-코드)를 적용하지 않고 항상 포함한다. 나머지
    launch_checklist 항목에는 소재·전원 신호 기반 필터를 적용한다.
    두 목록을 하나로 합쳐 리콜/일반 구분 없이 관련도 순으로 정렬 — 이전에는
    §5(리콜 기반)와 §7(일반)로 나뉘어 같은 성격의 체크항목이 반복되는 것처럼
    보였는데, 통합 후에는 "(리콜 발생함)" 표시로 출처만 구분한다.

    Returns:
        [(항목 텍스트, 리콜 발생 여부), ...] 관련도 순 정렬됨
    """
    signals = _build_checklist_signals(input_summary, legal_name)

    extra_items = _dedup_checklist(checklist, prevention_points)
    filtered_extra = _filter_conditional_items(extra_items, signals)

    combined = (
        [(p, True) for p in prevention_points]
        + [(i, False) for i in filtered_extra]
    )
    return sorted(combined, key=lambda t: -_score_checklist_item(t[0], signals))
...
def generate_markdown_report(response: DiagnosisResponse) -> str:
    ...
    if not cands:
        ...
        secondary_header_written = False
        for i, cand in enumerate(visible):
            ...
            if i == 0:
                ...
                # 2순위 이하는 위 CONFIRMED/CANDIDATE 결과와 혼동되지 않도록 명확히
                # "참고용·미사용" 구간으로 분리해서 표시한다. 실제 인증유형·체크리스트·
                # 리콜·KC 검색은 전부 최상위 후보(위 항목) 기준으로만 진행되며 이
                # 목록은 진단 결과에 반영되지 않는다.
                if not secondary_header_written:
                    md += (
                        "**낮은 신뢰도 후보 (참고용 — 아래 항목은 이번 진단 결과에 "
                        "사용되지 않았습니다)**\n\n"
                    )
                    secondary_header_written = True
                md += (
                    f"- {cand.display_product_name} — 신뢰도 {cand.confidence_score:.2f} "
                    f"(매칭 근거: {cand.match_basis})\n"
                )
        if hidden_count > 0:
            if not secondary_header_written:
                md += (
            ...
            md += f"- *그 외 매우 낮은 신뢰도 후보 {hidden_count}건은 생략됩니다.*\n"
        md += "\n"
    ...
    # 리콜 사유에서 도출된 위험 포인트는 §7 출시 전 확인 체크리스트에
    # "(리콜 발생함)" 표시로 통합되어 나타난다 (별도 중복 섹션 없음).
    ...
    # ── 7. 출시 전 확인 체크리스트 ───────────────────────────────────────
    # 리콜 이력에서 반복된 항목(prevention_points)과 나머지 공식 체크항목을
    # 하나로 통합 — 이전에는 §5(리콜 기반)·§7(일반)로 나뉘어 같은 안전기준
    # 체크항목 풀에서 나온 항목들이 서로 다른 섹션에 비슷한 문체로 반복
    # 표시되었는데, 통합 후에는 "(리콜 발생함)" 표시로 출처만 구분한다.
    md += "## 7. 출시 전 확인 체크리스트\n\n"

    if checklist or recall.prevention_points:
        unified = _build_unified_checklist(
            checklist, recall.prevention_points, response.input_summary, top_legal_name
        )
        for item, is_recall in unified:
            tag = " *(리콜 발생함)*" if is_recall else ""
            md += f"- [ ] {item}{tag}\n"
    else:

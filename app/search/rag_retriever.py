...
# 임베딩 코사인(보통 0.7~0.9대) 배율. rule boost(1.5~5)와 비슷한 스케일로 맞춤.
# 주의: min-max 등으로 재정규화하지 않고 원점수를 그대로 스케일만 함 —
# recall_service 블렌드 실측에서 min-max가 좁은 코사인 구간을 억지로 [0,1]로
# 늘리면서 노이즈까지 함께 증폭시키는 부작용을 확인했기 때문. 원점수를 쓰면
# 스케일은 작아도 실제 차이(진짜 관련도 차)만큼만 반영된다.
_EMBEDDING_BOOST_SCALE = 4.0
...
def _build_embedding_text(c: Dict[str, Any]) -> str:
    """chunk의 임베딩용 텍스트.

    BM25용 _build_doc_text와 달리 반복 가중(×2,×3)을 넣지 않는다 — 임베딩은
    단어 빈도가 아니라 문장 의미를 보므로 반복이 오히려 표현을 왜곡할 수 있다.
    """
    parts: List[str] = []
    if c["title"]:
        parts.append(c["title"])
    if c["product_category"]:
        parts.append(c["product_category"])
    if c["certification_type"]:
        parts.append(c["certification_type"])
    if c["keywords"]:
        parts.append(" ".join(c["keywords"]))
    if c["chunk_text"]:
        parts.append(c["chunk_text"][:600])
    return " ".join(parts)


class RagRetriever:
    """rag_chunk_all 전체에 대한 BM25 인덱스 + rule-aware 재랭킹 (+ 선택적 임베딩 보조)."""

    def __init__(self, raw_chunks: List[Dict[str, Any]], embedding_model: Any = None) -> None:
        ...
        # 임베딩 보조 인덱스 — chunk 수가 적고(수백 개) 고정 코퍼스라 구축 시
        # 1회만 전량 인코딩해 보관한다 (요청마다 재인코딩하지 않음).
        self._embedding_model = embedding_model
        self._chunk_embeddings = None
        if embedding_model is not None and self._chunks:
            try:
                if getattr(embedding_model, "available", False):
                    texts = [_build_embedding_text(c) for c in self._chunks]
                    mat = embedding_model.encode(texts, is_query=False)
                    if mat is not None:
                        self._chunk_embeddings = mat
                        logger.info("RAG chunk 임베딩 완료 (%d개)", len(self._chunks))
            except Exception:
                logger.exception("RAG chunk 임베딩 실패 — BM25만 사용")
    ...
    def embedding_available(self) -> bool:
        return self._chunk_embeddings is not None
    ...
    def retrieve(
        ...
        # 임베딩 보조 점수 (선택). 원점수(코사인)를 그대로 스케일만 해서 더함 —
        # 정규화하지 않아 좁은 코사인 구간이 노이즈로 증폭되지 않는다.
        emb_scores = None
        if self._chunk_embeddings is not None and self._embedding_model is not None:
            try:
                import numpy as np
                qv = self._embedding_model.encode_one(query_text, is_query=True)
                if qv is not None:
                    emb_scores = self._chunk_embeddings @ np.asarray(qv)
            except Exception as e:
                logger.warning("RAG 임베딩 쿼리 인코딩 실패 (BM25만 사용): %s", e)
                emb_scores = None
        ...
        for i, c in enumerate(self._chunks):
            ...
            if emb_scores is not None:
                boost += _EMBEDDING_BOOST_SCALE * float(emb_scores[i])
            total = base + boost
            if total <= 0:
                continue
